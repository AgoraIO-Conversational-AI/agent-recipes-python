...
endpoint — in this repo, the mounted `src/llm.py` app at `/llm` — via `CUSTOM_LLM_URL`. STT (Deepgram)
...
- `CUSTOM_LLM_URL` — the **public** chat-completions URL of your `llm/` endpoint
  (e.g. `https://<tunnel>/llm/chat/completions`). Agora cloud calls this directly, so
...
`src/llm.py` is the OpenAI-compatible `POST /chat/completions` mock Agora cloud
calls. It is mounted into this app at `/llm` (so the public URL is
`<tunnel>/llm/chat/completions`) and has no `agora-agents` dependency — it is the
part you replace with your own model. Run it in isolation with `python src/llm.py`
(default port 8001). Replace `get_mock_response()` to plug in a real model; a
production endpoint should validate the `Authorization: Bearer` header.
